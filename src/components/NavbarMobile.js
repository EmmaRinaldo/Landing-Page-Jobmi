...
function NavbarMobile() {
    ...
    return (
        <div className={`navbar navbar-sticky bg-[#F3F3F3] bg-opacity-70 backdrop-blur-lg shadow-none ${scrolled ? 'h-20' : 'h-24'} sm:${scrolled ? 'h-24' : 'h-28'} items-center sm:items-start transition-all duration-300`}>
            <div className="navbar-start flex flex-col sm:flex-row sm:h-full w-full">
                ...
                <a className="navbar-item flex items-center justify-center sm:justify-start py-4 pb-6 sm:py-6">
                    <img src={logoWhite} alt="Logo" className={`sm:hidden ${scrolled ? 'w-1/2' : 'w-2/3'} drop-shadow-md transition-all duration-300`} />
                </a>
            </div>
